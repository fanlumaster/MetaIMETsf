...
class CScrollBarWindow : public CBaseWindow
{
  public:
    ...
    BOOL _Create(ATOM atom, DWORD dwExStyle, DWORD dwStyle, CBaseWindow *pParent = nullptr, int wndWidth = 0,
                 int wndHeight = 0);
    ...
    virtual void _ShiftLine(int nLine, BOOL isNotify)
    {
        ...
        dwSB = (nLine > 0 ? SB_LINEDOWN : nLine < 0 ? SB_LINEUP : 0);
    ...
    virtual void _ShiftPage(int nPage, BOOL isNotify)
    {
        ...
        dwSB = (nPage > 0 ? SB_PAGEDOWN : nPage < 0 ? SB_PAGEUP : 0);
...
class CScrollBarNullWindow : public CScrollBarWindow
{
  public:
    ...
    virtual BOOL _Create(ATOM atom, DWORD dwExStyle, DWORD dwStyle, _In_opt_ CBaseWindow *pParent = nullptr,
    ...
    virtual LRESULT CALLBACK _WindowProcCallback(_In_ HWND wndHandle, _In_ UINT uMsg, _In_ WPARAM wParam,
                                                 _In_ LPARAM lParam)
    {
        wndHandle;
        uMsg;
